...
**淘汰赛进行中，已赛 76 场（72 小组赛 + 4 场 R32），准确率 68.4%（52/76），非平局 75.9%（41/54），平局 50.0%（11/22）。**
...
| **KO约束** (6/30) | 淘汰赛概率硬约束：平局≥25% + 胜率≤68% | 75场校准: P(H)>80%实际62.5%、P(D)<15%实际30-50% → 从第一性原理出发（弱队可摆大巴拖平局）|
...
| **→ 小组赛收官** | **69.4%** | 72场: 小组赛全部完成，最后12场8/12=66.7%，平局4/4全抓到，平局整体11/20=55.0% |

### 平局风险标注系统

- 风险 LOW/NONE → 模型判胜负可信度 ~78%（52场非平局中 39/52 正确）
- 风险 MED/HIGH → 平局概率约29%，胜负仅供参考
- 20场平局中：11场正确预测（含平局覆写）、4场被预警、5场完全无法预测

### 已知局限

1. **平局预测** — 分级阈值后抓到 11/20（55.0%），最后12场平局4/4全中。大差距平局仍有 9 场漏网（base P(D)太低，uplift 信号不够强）
2. **比分 MAE 1.86** — 泊松众数预测比分，极端比分（7:1）无法预测；72场比分精确率 13.9%（10/72）
3. **地缘权重待校准** — 伊朗-12pp、刚果(金)+2pp 均为经验估计，72场回测可开始反推最优参数
4. **淘汰赛样本尚小** — 4场R32 2/4正确，KO约束已上线（平局≥25%+胜率≤68%），待累积更多场次后回测校准

## 目录结构

```
data/raw/               # 原始数据
  matches_2026.csv      # 2026世界杯赛果（手动+自动更新，76场）
  schedule_2026.csv     # ★ 完整赛程（72场小组赛+16场1/16决赛）
...
scripts/                # 核心脚本
  ...
  calc_standings.py     # 小组积分榜 + 第三名排名计算
...
1. **淘汰赛逐场追踪** — 剩余 12 场 R32 + R16/QF/SF/Final，赛后回测 KO 约束有效性
2. **地缘政治权重校准** — 伊朗-12pp、刚果(金)+2pp 等参数在 76 场回测基础上反推最优值
3. **Motivation 层淘汰赛净化** — 小组赛资格形势（积分/出线/默契球）在淘汰赛应置零，目前仍对淘汰赛有微弱影响
