...
| **平局检测器** (6/18) | P(D)≥25% 且 draw_uplift≥0.04 → 覆写判平局 | 双方首轮均胜时触发 |
| **赔率融合** (6/18) | 模型 70% + 市场赔率 30% 加权 | 自动加载 odds_round*.csv |
...
1. **平局预测** — 泊松框架下平局永远不是概率最大值。平局检测器可在第二轮+覆写，但首轮无效
2. **比分 MAE 1.95** — 泊松众数预测比分，极端比分（7:1）无法预测
3. **地缘权重未校准** — 伊朗-12pp、刚果(金)+2pp 均为经验估计，待30+场后回测标定
4. **淘汰赛预测力未知** — 小组赛快结束，淘汰赛需要升级（休息天数差、加时点球）
...
1. 6/18 A+B 组第二轮 — 验证平局检测器+赔率融合首次实战
2. 累计 30+ 场后校准动机修正权重（回测反推最优参数）
3. 淘汰赛建模：加时→点球、单场淘汰心理因素
4. 淘汰赛对阵动态生成（schedule_2026.csv 目前只有小组赛）
5. ELO 替代 FIFA 排名 — 动态反映近期状态变化
