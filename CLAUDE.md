...
**已赛 60 场，预测准确率 66.7%（40/60），非平局 79.5%（35/44），平局 31.2%（5/16）。**
...
1. **平局预测** — 泊松框架下平局永远不是概率最大值。平局检测器抓到 5/16（31.2%），第三轮默契球多但检测率有所提升。悬殊 ELO 和首轮的平局仍无法预测
2. **比分 MAE 1.90** — 泊松众数预测比分，极端比分（7:1）无法预测；60场后比分精确率 11.7%（7/60）
...
1. 小组赛最后 12 场（6/25-6/26）等 Wikipedia 更新 → 回测最终准确率
2. 淘汰赛 32 强对阵确认 → 赔率 + 预测
3. 淘汰赛建模升级：加时→点球、单场淘汰心理、休息天数差权重放大
4. 60 场回测标定地缘政治权重（伊朗-12pp、刚果(金)+2pp 等经验值反推最优参数）
